...
for i in dicts_creds.items():
    ...
    for i in range(len(list_of_dfs)):
        if i == 0:
            ...
            df = df.merge(list_of_dfs[i], on=['date', 'itemBrand', 'itemId', 'itemName', 'country_source'], how='outer')
...
df = df[['brand', 'country', 'date', 'country_source', 'itemBrand', 'itemId', 'itemName',
         'activeUsers', 'bounceRate', 'cartToViewRate', 'itemsAddedToCart',
         'itemsCheckedOut', 'grossItemRevenue', 'itemRevenue', 'engagedSessions',
         'engagementRate', 'sessionConversionRate',
         'sessions', 'totalUsers', 'userConversionRate']]
...
df.to_sql(f'table_name', con=engine, if_exists='replace', index=False, chunksize=1000,
          dtype={'itemName': types.UnicodeText(),
                 'itemBrand': types.UnicodeText(), })
