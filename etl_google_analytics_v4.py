...
email = "starting-account-lgmlmgtpzywl@ga-python-1675759321449.iam.gserviceaccount.com"
...
start_date = (dt.datetime.today() - dt.timedelta(days=60)).strftime("%Y-%m-%d")
...
for i in dicts_creds.items():
    ...
    print(brand, country, property_id, acc_type)
    ...
    for j in range(int(len(metric_list) / 10) + 1):
        ...
        df_temp.rename(columns={'country': 'country_source'}, inplace=True)
        if j == 0:
            df_temp["brand"] = brand
            df_temp["country"] = country
        list_of_dfs.append(df_temp)
        df_base = pd.DataFrame(
            pd.date_range(start=start_date, end=(dt.datetime.now() - dt.timedelta(days=1)).strftime("%Y-%m-%d"),
                          freq="D"), columns=["date"])
    ...
    for i in range(len(list_of_dfs)):
        if i == 0:
            ...
            df = df.merge(list_of_dfs[i], on=['date', 'sessionCampaignName',
                                              "sessionSource",
                                              "sessionMedium", "country_source"], how='outer')
...
df = df[['brand', 'country', 'date', 'country_source', 'sessionCampaignName', 'sessionSource',
         'sessionMedium', 'activeUsers', 'addToCarts', 'averageSessionDuration',
         'bounceRate', 'cartToViewRate', 'checkouts', 'conversions',
         'engagedSessions', 'engagementRate', 'screenPageViews', 'purchaseRevenue', 'sessionConversionRate', 'sessions',
         'totalPurchasers', 'totalAdRevenue', 'totalRevenue', 'totalUsers',
         'userConversionRate', 'currency', 'property_id', 'type']]

df.rename(columns={'country_source': 'countrySource'}, inplace=True)
...
# cursor.callproc('dwh.update_ga_metrics_rpt')
